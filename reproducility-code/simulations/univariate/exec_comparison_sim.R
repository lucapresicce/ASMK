...
rm(list = ls())
gc()
setwd("~/MetaApproaches/Simulations")
# Packages --------------------------------------------------------------------
library(ASMK)
library(Rcpp)
...
fit_loop <- function(i) {
  ...
  bps <- ASMK::BPS_weights(data = list(Y = Yi, X = Xi),
                           priors = list(mu_b = matrix(rep(0, p)),
                                         ...
                                         a = 2,
                                         b = 2), coords = crd_i,
                           hyperpar = list(delta = delta_seq, phi = phi_seq), K = 5, g = 5)
  w_hat <- bps$W
  epd <- bps$epd
...
#   ind_s <- subset_ind[r]
#   Ys <- matrix(data_part$Y_list[[ind_s]]); Xs <- data_part$X_list[[ind_s]]; crds <- data_part$crd_list[[ind_s]]; Ws <- W_list[[ind_s]]
#   result <- ASMK::BPS_pred(data = list(Y = Ys, X = Xs), coords = crds,
#                            X_u = X_u, crd_u = crd_u,
#                            priors = list(mu_b = matrix(rep(0, p)),
#                                          V_b = diag(10, p),
#                                          a = 2,
...
# 
# # function for the pred loop
# post_loop <- function(r) {
#   
#   ind_s <- subset_ind[r]
#   Ys <- matrix(data_part$Y_list[[ind_s]]); Xs <- data_part$X_list[[ind_s]]; crds <- data_part$crd_list[[ind_s]]; Ws <- W_list[[ind_s]]
#   result <- ASMK::BPS_postdraws(data = list(Y = Ys, X = Xs), coords = crds,
#                                 priors = list(mu_b = matrix(rep(0, p)),
#                                               V_b = diag(10, p),
#                                               a = 2,
#                                               b = 2),
...
# # list of function
# funs_post <- lsf.str()[which(lsf.str() != "post_loop")]
...
# # posterior sample
# M <- 250
# subset_ind <- sample(1:K, M, T, Wbps)
# tic("post")
# post_smp_asmk <- foreach(r = 1:M, .noexport = funs_post) %dopar% { post_loop(r) }
# post_time <- toc()
...
# Plotting 
png("comp_sim_15_500.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
par(mfrow = c(1, 2))
iw <- as.image.SpatialGridDataFrame(surf.W)
iy <- as.image.SpatialGridDataFrame(surf.Y)
...
# Plotting 
png("comp_res_15_500.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
par(mfrow = c(1, 2))
iwp <- as.image.SpatialGridDataFrame(surf.Wp)
iyp <- as.image.SpatialGridDataFrame(surf.Yp)
...
png("comp_cover_15_500.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
cowplot::plot_grid(uc_W, uc_Y)
dev.off()
...
# Plotting 
png("comp_comp_15_500.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
par(mfrow = c(1, 3))
iysmk <- as.image.SpatialGridDataFrame(surf.Ysmk)
plot(crd, type="n", cex=0.5, xlim=xlim, axes=FALSE, ylab="Northing", xlab="Easting",
...
png("comp_UCcomp_15_500.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
cowplot::plot_grid(uc_Yasmk, uc_Ysmk)
dev.off()
...
rm(list = ls()[which(!(ls() %in% c("results")))])
save.image(file = "COMPARISON_15_500.RData")
# load("COMPARISON_15_500.RData")


#####################################################################################################################################################
## ASMK COMPARISON - 15K - 1000 ############################################
rm(list = ls())
gc()
setwd("~/MetaApproaches/Simulations")
# Packages --------------------------------------------------------------------
...
# Plotting 
png("comp_sim_15_1000.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
par(mfrow = c(1, 2))
iw <- as.image.SpatialGridDataFrame(surf.W)
iy <- as.image.SpatialGridDataFrame(surf.Y)
...
# Plotting 
png("comp_res_15_1000.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
par(mfrow = c(1, 2))
iwp <- as.image.SpatialGridDataFrame(surf.Wp)
iyp <- as.image.SpatialGridDataFrame(surf.Yp)
...
png("comp_cover_15_1000.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
cowplot::plot_grid(uc_W, uc_Y)
dev.off()
...
# Plotting 
png("comp_comp_15_1000.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
par(mfrow = c(1, 3))
iysmk <- as.image.SpatialGridDataFrame(surf.Ysmk)
plot(crd, type="n", cex=0.5, xlim=xlim, axes=FALSE, ylab="Northing", xlab="Easting",
...
png("comp_UCcomp_15_1000.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
cowplot::plot_grid(uc_Yasmk, uc_Ysmk)
dev.off()
...
rm(list = ls()[which(!(ls() %in% c("results")))])
save.image(file = "COMPARISON_15_1000.RData")
# load("COMPARISON_15_1000.RData")
...
# posterior_asmk <- as.matrix(do.call(rbind, res_5_500$ASMK$post))
...
# mcmc_recover_hist(matrix(posterior_asmk[,1], dimnames = list(NULL,"sigma^2")), true_par[1],
#                   facet_args = list(labeller = ggplot2::label_parsed), binwidth = 0.1)
# mcmc_recover_hist(posterior_asmk[,2:5], true_par[2:5],
#                   facet_args = list(labeller = ggplot2::label_parsed), binwidth = 0.05)

# mcmc_recover_scatter(posterior_asmk, true_par, point_est = "mean")
...
# mcmc_recover_hist(matrix(posterior_smk[,1], dimnames = list(NULL,"sigma2")), true_par[1],
#                   facet_args = list(labeller = ggplot2::label_parsed), binwidth = 0.1)
# mcmc_recover_hist(posterior_smk[,2:5], true_par[2:5],
#                   facet_args = list(labeller = ggplot2::label_parsed), binwidth = 0.05)

# mcmc_recover_scatter(posterior_smk, true_par, point_est = "mean")
...
# timing
cat("ASMK timing : \n"); round(res_15_500$ASMK$time/60, 2)
cat("SMK timing : \n"); round(res_15_500$SMK$time/60, 2)
cat("Relative timing ratio SMK over ASMK : \n"); round(as.numeric(as.numeric(round(res_15_500$SMK$time/60, 2))[4] / round(res_15_500$ASMK$time/60, 2)[4]), 2)
...
# posterior samples collection
# posterior_asmk <- as.matrix(do.call(rbind, res_5_500$ASMK$post))
posterior_asmk <- t(res_15_500$ASMK$post)
posterior_smk <- cbind(res_15_500$SMK$post[,5], res_15_500$SMK$post[,1:4])
colnames(posterior_asmk) <- colnames(posterior_smk) <- c("sigma^2", "beta[1]", "beta[2]", "beta[3]", "beta[4]")
...
color_scheme_set("viridis")
mcmc_recover_intervals(posterior_asmk, true_par,
                       prob = 0.89,
                       prob_outer = 0.95,
                       point_est = "mean",
                       size = 4,
...
color_scheme_set("viridis")
mcmc_recover_intervals(posterior_smk, true_par,
                       prob = 0.89,
                       prob_outer = 0.95,
                       point_est = "mean",
                       size = 3,
...
# timing
cat("ASMK timing : \n"); round(res_15_1000$ASMK$time/60, 2)
cat("SMK timing : \n"); round(res_15_1000$SMK$time/60, 2)
cat("Relative timing ratio SMK over ASMK : \n"); round(as.numeric(as.numeric(round(res_15_1000$SMK$time/60, 2))[4] / round(res_15_1000$ASMK$time/60, 2)[4]), 2)
...
# posterior samples collection
# posterior_asmk <- as.matrix(do.call(rbind, res_5_500$ASMK$post))
posterior_asmk <- t(res_15_1000$ASMK$post)
posterior_smk <- cbind(res_15_1000$SMK$post[,5], res_15_1000$SMK$post[,1:4])
colnames(posterior_asmk) <- colnames(posterior_smk) <- c("sigma^2", "beta[1]", "beta[2]", "beta[3]", "beta[4]")
