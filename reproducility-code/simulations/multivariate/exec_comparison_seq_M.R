...
rm(list = ls())
gc()
setwd("~/MetaApproaches/SimulationsM")
...
for (i in 1:K) {
  ...
  out <- fit_latent_cpp(data = list(Y = data_part$Y_list[[i]], X = data_part$X_list[[i]]),
  ...
  smp <- post_draws_latent(out, R, F, p)
...
tic()
predictions_ws <- sapply(1:R, function(r) {r_pred_latent_cpp(data = list(Y = YK, X = XK), X_u = X_u,
                                                             d_u = arma_dist(crd_u), d_us = arma_dist(rbind(crd_u, crd_K)),
                                                             hyperpar = hpar, poster = out, beta = smp[[r]]$beta, sigma = smp[[r]]$sigma)$Yu},
                      simplify = "array")
toc()
...
# zlim1 <- range(c(surf.Y1$z, surf.Y1hat$z))
# zlim2 <- range(c(surf.Y2$z, surf.Y2hat$z))
# zlim3 <- range(c(surf.Y3$z, surf.Y3hat$z))
...
YK <- data_part$Y_list[[K]]; XK <- data_part$X_list[[K]]; crd_K <- data_part$crd_list[[K]]
tic()
predictions_ms <- sapply(1:R, function(r) {r_pred_latent_cpp(data = list(Y = YK, X = XK), X_u = X_u,
                                                             d_u = arma_dist(crd_u), d_us = arma_dist(rbind(crd_u, crd_K)),
                                                             hyperpar = hpar, poster = out, beta = smp[[r]]$beta, sigma = smp[[r]]$sigma)$Yu},
                      simplify = "array")
toc()
...
YK <- data_part$Y_list[[K]]; XK <- data_part$X_list[[K]]; crd_K <- data_part$crd_list[[K]]
tic()
predictions_hms <- sapply(1:R, function(r) {r_pred_latent_cpp(data = list(Y = YK, X = XK), X_u = X_u,
                                                              d_u = arma_dist(crd_u), d_us = arma_dist(rbind(crd_u, crd_K)),
                                                              hyperpar = hpar, poster = out, beta = smp[[r]]$beta, sigma = smp[[r]]$sigma)$Yu},
                      simplify = "array")
toc()
