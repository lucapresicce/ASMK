...
Sys.setenv(PKG_CXXFLAGS = "-Ofast"); sourceCpp("~/ASMK/src/code.cpp")
library(mniw)
library(MCMCpack)
library(ggplot2)
library(tictoc)
...
tic()
predictions <- r_pred_cpp(data = list(Y = YK, X = XK), X_u = X_u, iRphi_s = out$iRphi_s,
                          d_u = arma_dist(crd_u), d_us = arma_dist(rbind(crd_u, crd_K)),
                          hyperpar = hpar, poster = smp)
toc()

# pred_mat_W <- predictions$Z_u
...
# d.max <- sqrt((max(crd[,1]) - min(crd[,1]))^2 + 
#                 (max(crd[,2]) - min(crd[,2]))^2)
# d.max # around 1.41
# 
# # check the variogram 
# library(geoR)
# v.resid <- variog(coords = crd_s, data = resid(lm.obj), 
#                   uvec = (seq(0, d.max, length = 50)))
# vario.fit <- variofit(v.resid, cov.model="exponential")
# plot(v.resid, xlab = "Distance", cex = 0.1, lwd = 5)
# lines(vario.fit, col = 2, lwd = 2) 
# abline(h = c(vario.fit$nugget, vario.fit$nugget + vario.fit$cov.pars[1]))
# abline(v = 3 * vario.fit$cov.pars[2])
# 
# # set hyperparameter grid from EDA
# summary(vario.fit)
# delta_hat <- vario.fit$nugget/vario.fit$cov.pars[1]
# delta_step <- (delta_hat)/3
# delta_seq <- seq(-delta_step, delta_step, delta_step) + delta_hat
# phi_hat <- 1 / vario.fit$cov.pars[2]
# phi_step <- (1 / vario.fit$cov.pars[2])/3
# phi_seq <- seq(-phi_step, phi_step, phi_step) + phi_hat
...
fit_loop <- function(i) {
  ...
  bps <- ASMK::BPS_weights(data = list(Y = Yi, X = Xi),
                           priors = list(mu_b = matrix(rep(0, p)),
                                         ...
                                         a = 2,
                                         b = 2), coords = crd_i,
                           hyperpar = list(delta = delta_seq, phi = phi_seq), K = 5, g = 20)
  w_hat <- bps$W
  epd <- bps$epd
